...
var _chunkWUJHPAYAjs = require('./chunk-WUJHPAYA.js');
...
// node/cli.ts
var import_fast_deep_equal = _chunkGE7W2DBEjs.__toESM.call(void 0, _chunkZVE3I55Ejs.require_fast_deep_equal.call(void 0, ));
...
_fs.injectPreparserExtensionLoader.call(void 0, async (headmatter, filepath) => {
  ...
  const roots = (
    /* uniq */
    [
      _chunkWUJHPAYAjs.getUserRoot.call(void 0, {}).userRoot,
      ..._chunkWUJHPAYAjs.getAddonRoots.call(void 0, addons, ""),
      _chunkWUJHPAYAjs.getClientRoot.call(void 0, )
    ]
  );
  const mergeArrays = (a, b) => a.concat(b);
  return await _chunkZVE3I55Ejs.loadSetups.call(void 0, roots, "preparser.ts", { filepath, headmatter }, [], false, mergeArrays);
});
var cli = _yargs2.default.scriptName("slidev").usage("$0 [args]").version(_chunkWUJHPAYAjs.version).strict().showHelpOnFail(false).alias("h", "help").alias("v", "version");
cli.command(
  "* [entry]",
  ...
  async ({ entry, theme, port: userPort, open, log, remote, tunnel, force, inspect }) => {
    ...
    async function initServer() {
      if (server)
        await server.close();
      const options = await _chunkWUJHPAYAjs.resolveOptions.call(void 0, { entry, remote, theme, inspect }, "dev");
      port = userPort || await findFreePort(3030);
      server = await _chunkWUJHPAYAjs.createServer.call(void 0, 
        options,
        {
          server: {
            port,
        ...
        },
        {
          onDataReload(newData, data) {
            if (!theme && _chunkWUJHPAYAjs.resolveThemeName.call(void 0, newData.config.theme) !== _chunkWUJHPAYAjs.resolveThemeName.call(void 0, data.config.theme)) {
...
cli.command(
  ...
  async (args) => {
    ...
    const { build } = await Promise.resolve().then(() => require("./build-OAINTTBE.js"));
    for (const entryFile of entry) {
      const options = await _chunkWUJHPAYAjs.resolveOptions.call(void 0, { entry: entryFile, theme, inspect }, "build");
...
cli.command(
  ...
  async ({ entry }) => {
    for (const entryFile of entry) {
      const data = await _chunkWUJHPAYAjs.parser.load(entryFile);
      _chunkWUJHPAYAjs.parser.prettify(data);
      await _chunkWUJHPAYAjs.parser.save(data);
    }
  }
);
cli.command(
  ...
  (command) => {
    return command.command(
      ...
      async ({ entry, dir, theme: themeInput }) => {
        const data = await _chunkWUJHPAYAjs.parser.load(entry);
        const theme = _chunkWUJHPAYAjs.resolveThemeName.call(void 0, themeInput || data.config.theme);
        if (theme === "none") {
          console.error('Cannot eject theme "none"');
          process.exit(1);
        }
        if (_chunkWUJHPAYAjs.isPath.call(void 0, theme)) {
          console.error("Theme is already ejected");
          process.exit(1);
        }
        const roots = _chunkWUJHPAYAjs.getThemeRoots.call(void 0, theme, entry);
        if (!roots.length) {
...
cli.command(
  ...
  async (args) => {
    ...
    for (const entryFile of entry) {
      const options = await _chunkWUJHPAYAjs.resolveOptions.call(void 0, { entry: entryFile, theme }, "export");
      const server = await _chunkWUJHPAYAjs.createServer.call(void 0, 
        options,
        {
          server: { port },
          clearScreen: false
      ...
      printInfo(options);
      _chunkWUJHPAYAjs.parser.filterDisabled(options.data);
...
cli.command(
  ...
  async ({
    ...
    const port = await findFreePort(12445);
    for (const entryFile of entry) {
      const options = await _chunkWUJHPAYAjs.resolveOptions.call(void 0, { entry: entryFile }, "export");
      const server = await _chunkWUJHPAYAjs.createServer.call(void 0, 
        options,
        {
          server: { port },
...
function printInfo(options, port, remote, tunnelUrl) {
  ...
  console.log(`${_kolorist.bold.call(void 0, "  Slidev")}  ${_kolorist.blue.call(void 0, `v${_chunkWUJHPAYAjs.version}`)} ${_isinstalledglobally2.default ? _kolorist.yellow.call(void 0, "(global)") : ""}`);
  console.log();
